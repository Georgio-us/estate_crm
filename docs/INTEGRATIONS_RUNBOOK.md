...
CRM roles cap delivery scope. Admins and team leads may subscribe to organization-wide activity; managers can receive only notifications assigned to them. The message intentionally contains no lead form fields or telephone number: only `Новый лид · #<номер сделки>` / `Повторное обращение · #<номер сделки>` and an **Открыть лид** button that targets `/?deal=<deal-id>` in the CRM. Older queued notifications created before deal numbers were added remain deliverable with the previous short title.

Each connected user has independent Telegram preferences. Admins and team leads can choose **all team members**, **only me**, **selected team members**, or **do not send**. Managers are server-limited to **only me** or **do not send**. Lead, upcoming-task, and overdue-task notifications can be enabled independently. These settings affect delivery only; they do not change which CRM records the role is allowed to open. By default a newly connected admin watches the whole organization; a newly connected team lead or manager watches only their own work.

Assignment produces an immediate short task notification. The Telegram runtime also scans active dated tasks every 30 seconds: a task with an exact time produces one reminder during the final 30 minutes before its deadline and one overdue notification after the deadline. A date-only task becomes overdue at the end of that local organization day. Task notifications carry the assignee, deadline, task ID, and deal number in their internal payload; the visible message stays short and opens `/?deal=<deal-id>&task=<task-id>` (or `/tasks?task=<task-id>` for a task without a deal). Stable deduplication keys prevent repeated pushes. Editing, completing, or rescheduling a task cancels its pending old notification; reassignment creates a fresh message for the new assignee.
