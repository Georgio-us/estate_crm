...
export function PropertyDrawer({ property, onSave, onClose }: { property: PropertyListing; onSave: (property: PropertyListing) => Promise<void>; onClose: () => void }) {
      ...
      <aside className={styles.drawer} role="dialog" aria-modal="true" aria-label={`Объект ${property.title}`}>
        ...
        <div className={styles.drawerBody}>
          <div className={styles.visualPane}>
            ...
            <section className={styles.description}><h3>Описание</h3><textarea className={styles.drawerTextarea} value={draft.description} onChange={(event) => update({ description: event.target.value })} placeholder="Описание объекта" /></section>
          </div>
