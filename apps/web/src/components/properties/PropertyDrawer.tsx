        ...
        <div className={styles.drawerBody}>
          ...
          <div className={styles.propertyPane}>
            ...
            {readOnly ? <ReadOnlyFacts property={draft} /> : <><section className={styles.factSection}>
              ...
              <EditField label="Валюта"><select value={draft.currency} onChange={(event) => update({ currency: event.target.value as PropertyListing["currency"] })}><option value="USD">USD — доллар</option><option value="UAH">UAH — гривна</option><option value="EUR">EUR — евро</option></select></EditField>
              <EditField label="Общая цена"><input inputMode="decimal" value={draft.price ?? ""} onChange={(event) => update({ price: event.target.value ? Number(event.target.value) : null })} /></EditField>
              <EditField label="Цена за м²"><input inputMode="decimal" value={draft.pricePerSquareMeter ?? ""} onChange={(event) => { const pricePerSquareMeter = event.target.value ? Number(event.target.value) : null; update({ pricePerSquareMeter, ...(pricePerSquareMeter !== null && draft.area !== null ? { price: Math.round(pricePerSquareMeter * draft.area * 100) / 100 } : {}) }); }} /></EditField>{draft.priceRaw && <p>Исходное значение: {draft.priceRaw}</p>}
              ...
              <EditField label="Площадь"><input inputMode="decimal" value={draft.area ?? ""} onChange={(event) => { const area = event.target.value ? Number(event.target.value) : null; update({ area, ...(area !== null && draft.pricePerSquareMeter ? { price: Math.round(area * draft.pricePerSquareMeter * 100) / 100 } : {}) }); }} /></EditField>{draft.areaRaw && <p>Исходное значение: {draft.areaRaw}</p>}
