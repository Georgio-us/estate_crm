    ...
    <div className={styles.modalLayer}>
      ...
      <form className={styles.modal} onSubmit={(event) => { event.preventDefault(); void submit(); }} onKeyDown={(event) => { const target = event.target as HTMLElement; if (event.key === "Enter" && target.tagName !== "TEXTAREA" && target.getAttribute("type") !== "submit") event.preventDefault(); }}>
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.formGrid}><Field label="Источник"><select value={draft.source} onChange={(event) => update({ source: event.target.value as Deal["source"] })}><option value="Manual">Не указан</option><option value="Meta">Meta</option><option value="Website">Сайт</option><option value="Call">Звонок</option><option value="Referral">Рекомендация</option></select></Field><Field label="Ответственный"><select value={draft.assigneeId} onChange={(event) => update({ assigneeId: event.target.value })}><option value="">Не назначен</option>{assignees.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></Field></div>
