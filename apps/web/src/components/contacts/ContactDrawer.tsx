...
export function ContactDrawer({ contact, contacts, deals, activities, assignees, onSave, onAddNote, onLinkContact, onUnlinkContact, onCreateDeal, onClose }: { contact: Contact; contacts: Contact[]; deals: RelatedDeal[]; activities: ActivityEvent[]; assignees: Array<{ id: string; name: string }>; onSave: (contact: Contact) => Promise<Contact>; onAddNote: (text: string) => Promise<void>; onLinkContact: (contactId: string, label?: string) => Promise<void>; onUnlinkContact: (contactId: string) => Promise<void>; onCreateDeal: () => void; onClose: () => void }) {
