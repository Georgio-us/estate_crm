...
export function ContactDrawer({ contact, contacts, deals, activities, assignees, canManage, onSave, onAddNote, onLinkContact, onUnlinkContact, onCreateDeal, onCreateTask, onArchive, onRestore, onDelete, onClose }: { contact: Contact; contacts: Contact[]; deals: RelatedDeal[]; activities: ActivityEvent[]; assignees: Array<{ id: string; name: string }>; canManage: boolean; onSave: (contact: Contact) => Promise<Contact>; onAddNote: (text: string) => Promise<void>; onLinkContact: (contactId: string, label?: string) => Promise<void>; onUnlinkContact: (contactId: string) => Promise<void>; onCreateDeal: () => void; onCreateTask: () => void; onArchive: () => Promise<void>; onRestore: () => Promise<void>; onDelete: () => Promise<void>; onClose: () => void }) {
    ...
    <div className={styles.drawerLayer}>
      ...
      <aside className={styles.drawer} role="dialog" aria-modal="true" aria-label={`Контакт ${contact.name}`}>
        <header className={styles.drawerHeader}>
          ...
          <div className={styles.headerActions}><div className={styles.contactMenuWrap}><button type="button" aria-label="Меню контакта" aria-expanded={menuOpen} onClick={() => setMenuOpen((current) => !current)}>•••</button>{menuOpen && <div className={styles.contactMenu}>{canManage ? contact.status === "ACTIVE" ? <button type="button" onClick={() => { setMenuOpen(false); void onArchive(); }}>Перенести в архив</button> : <><button type="button" onClick={() => { setMenuOpen(false); void onRestore(); }}>Восстановить контакт</button><button className={styles.menuDanger} type="button" onClick={() => { setMenuOpen(false); void onDelete(); }}>Удалить навсегда</button></> : <span>Управление архивом доступно руководителю</span>}</div>}</div><button type="button" onClick={onClose} aria-label="Закрыть">×</button></div>
        </header>
        <div className={styles.drawerWorkspace}>
          <div className={styles.detailsPane}>
            ...
            <section className={styles.dealsSection}><div className={styles.sectionTitle}><h3>Связанные сделки</h3><span>{deals.length}</span></div>{deals.length ? deals.map(({ deal, stageTitle, stageColor }) => <Link className={styles.dealRow} href={`/?deal=${deal.id}`} aria-label={`Открыть сделку ${deal.number}`} key={deal.id}><span className={styles.dealColor} style={{ background: stageColor }} /><div><strong>{deal.title || deal.request}</strong><small>Сделка #{deal.number} · {stageTitle}</small></div><span>{deal.budget || "Без бюджета"}</span></Link>) : <div className={styles.noDeals}>У контакта пока нет сделок</div>}<button className={styles.addRelationButton} type="button" onClick={onCreateDeal}>＋ Создать сделку</button></section>
