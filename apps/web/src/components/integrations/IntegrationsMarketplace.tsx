...
import { ViaConnectionPanel } from "./ViaConnectionPanel";
import styles from "./integrations.module.css";

type IntegrationProvider = "TEST" | "META_LEAD_ADS" | "INSTAGRAM_DIRECT" | "TELEPHONY" | "TELEGRAM" | "VIA";
type IntegrationConnectionRecord = { provider: IntegrationProvider; status: "READY" | "CREDENTIALS_REQUIRED" | "CONNECTED" | "ERROR"; enabled: boolean; pipelineId: string | null; stageId: string | null; lastEventAt: string | null; lastError: string | null; processedCount: number; webhookConfigured: boolean };
...
const integrations: Integration[] = [
  ...
  { id: "via", provider: "VIA", name: "Via · подборки в Telegram", category: "Данные", description: "Подключаемый клиентский каталог и ссылки на подборки из сделок CRM.", icon: "properties", brand: "via", available: true },
...
const providerNames: Record<IntegrationProvider, string> = { TEST: "Тестовый шлюз", META_LEAD_ADS: "Facebook Lead Ads", INSTAGRAM_DIRECT: "Instagram Direct", TELEPHONY: "Телефония", TELEGRAM: "Telegram", VIA: "Via" };
...
function IntegrationPanel({ item, state, onCreated, onClose }: { item: Integration; state?: IntegrationConnectionRecord; onCreated: () => Promise<void>; onClose: () => void }) {
  const [provider, setProvider] = useState<"TEST" | "META_LEAD_ADS" | "INSTAGRAM_DIRECT" | "TELEPHONY">(item.provider && item.provider !== "TELEGRAM" && item.provider !== "VIA" ? item.provider : "TEST");
  ...
  const [setup, setSetup] = useState<GoogleSheetsSetup | null>(null);
  ...
  async function setupGoogleSheets() { setBusy(true); setFeedback(""); try { const response = await fetch("/api/crm/integrations/meta-lead-ads/google-sheets/setup", { method: "POST" }); const payload = await response.json() as GoogleSheetsSetup & { message?: string }; if (!response.ok || !payload.secret) throw new Error(payload.message || "Не удалось создать подключение."); setSetup(payload); setFeedback("Ключ создан. Он показывается только сейчас — после переноса в Apps Script обновлять его не нужно."); await onCreated(); } catch (caught) { setFeedback(caught instanceof Error ? caught.message : "Ошибка подключения."); } finally { setBusy(false); } }
  async function connectTelegram() { setBusy(true); setFeedback(""); try { const response = await fetch("/api/crm/integrations/telegram/connect", { method: "POST" }); const payload = await response.json() as TelegramSetup & { message?: string }; if (!response.ok || !payload.connectUrl) throw new Error(payload.message || "Не удалось создать ссылку подключения."); setTelegramSetup(payload); window.location.assign(payload.connectUrl); } catch (caught) { setFeedback(caught instanceof Error ? caught.message : "Ошибка подключения Telegram."); } finally { setBusy(false); } }
  const canTest = item.provider !== "TELEGRAM" && item.provider !== "VIA" && item.id !== "csv";
  return <div className={styles.panelLayer}><button className={styles.backdrop} type="button" aria-label="Закрыть" onClick={onClose} /><aside className={styles.panel}><header><BrandMark item={item} /><span><small>{item.category}</small><h2>{item.name}</h2></span><button type="button" aria-label="Закрыть" onClick={onClose}>×</button></header><div className={styles.panelBody}><StatusBadge item={item} state={state} /><p>{item.description}</p>
    ...
    {item.id === "telegram" && <TelegramConnectionPanel busy={busy} telegramSetup={telegramSetup} onConnect={connectTelegram} />}
    {item.id === "via" && <ViaConnectionPanel onUpdated={onCreated} />}
...
function TelegramConnectionPanel({ busy, telegramSetup, onConnect }: { busy: boolean; telegramSetup: TelegramSetup | null; onConnect: () => Promise<void> }) {
  const [connected, setConnected] = useState<boolean | null>(null);
  useEffect(() => {
    const controller = new AbortController();
    void fetch("/api/crm/integrations/telegram/preferences", { cache: "no-store", signal: controller.signal })
      .then(async (response) => { const payload = await response.json() as { connected?: boolean }; if (response.ok) setConnected(Boolean(payload.connected)); else setConnected(false); })
      .catch(() => { if (!controller.signal.aborted) setConnected(false); });
    return () => controller.abort();
  }, []);
  return <>
    <section className={styles.installInfo}><h3>{connected === null ? "Проверяем подключение…" : connected ? "Ваш Telegram подключён" : "Подключить Telegram"}</h3><p>Здесь CRM только связывает ваш профиль с ботом. Десять минут действует сама одноразовая ссылка; после активации подключение остаётся постоянным.</p><div><span>✓ Один бот для всей CRM</span><span>✓ Персональная связь с сотрудником</span><span>✓ Короткие ссылки на сделки и задачи</span><button className={styles.panelPrimary} disabled={busy} type="button" onClick={() => { void onConnect(); }}>{busy ? "Создаём ссылку…" : connected ? "Подключить другой Telegram" : "Подключить Telegram"}</button>{telegramSetup && <small>Если Telegram не открылся автоматически, <a href={telegramSetup.connectUrl}>откройте бота</a>.</small>}</div></section>
    <section className={styles.installInfo}><h3>Настройки уведомлений находятся в команде</h3><p>Охват и виды уведомлений управляются в профиле сотрудника, а не в технической карточке подключения.</p><Link className={styles.teamSettingsLink} href="/team">Открыть команду →</Link></section>
  </>;
}
