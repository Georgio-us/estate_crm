...
import type { TaskContactOption, TaskDealOption, TaskDraftInput } from "./TasksContext";
...
export function NewTaskModal({ contacts, deals, assignee, initialDate, initialContactId, initialDealId, onCreate, onClose }: NewTaskModalProps) {
  const initialDeal = deals.find((deal) => deal.id === initialDealId);
  const [draft, setDraft] = useState<NewTaskDraft>({ title: "", kind: "Звонок", dueDate: initialDate || localDateKey(), dueTime: "", assigneeId: assignee.id, contactId: initialContactId || initialDeal?.contactId, dealId: initialDealId });
