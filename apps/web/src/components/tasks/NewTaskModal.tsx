...
import type { CrmTaskType } from "@/types/crm";
...
export function NewTaskModal({ contacts, deals, assignees, taskTypes, currentUserId, initialDate, initialContactId, initialDealId, onCreate, onClose }: NewTaskModalProps) {
  ...
  const defaultTaskType = taskTypes.find((item) => item.isActive);
  const [draft, setDraft] = useState<NewTaskDraft>({ title: "", kind: defaultTaskType?.baseKind ?? "Звонок", taskTypeId: defaultTaskType?.id, dueDate: initialDate || localDateKey(), dueTime: "", assigneeId: currentUserId, contactId: initialContactId || initialDeal?.contactId, dealId: initialDealId || (initialContactDeals.length === 1 ? initialContactDeals[0].id : undefined) });
