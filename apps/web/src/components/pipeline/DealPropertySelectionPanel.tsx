...
export function DealPropertySelectionPanel({ deal, onClose, onCountChange, onActivityChanged }: { deal: Deal; onClose: () => void; onCountChange: (count: number) => void; onActivityChanged: () => Promise<void> }) {
  ...
  async function revokeShare(share: PropertySelectionShareRecord) {
    setBusyKey(share.id); setError("");
    try {
      const response = await fetch(`/api/crm/deals/${deal.id}/property-shares/${share.id}/revoke`, { method: "POST" });
      const payload = await response.json() as { share?: PropertySelectionShareRecord; message?: string };
      if (!response.ok || !payload.share) throw new Error(payload.message || "Не удалось отозвать ссылку.");
      setShares((current) => current.map((item) => item.id === share.id ? payload.share! : item)); await onActivityChanged();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Не удалось отозвать ссылку."); } finally { setBusyKey(null); }
  }
    ...
    <aside className={styles.selectionDrawer} role="dialog" aria-modal="true" aria-label="Подборка объектов">
      ...
      {loadState === "loading" ? <div className={styles.selectionState}>Загружаем подборку…</div> : loadState === "error" ? <div className={styles.selectionState}><p>{error}</p><button type="button" onClick={() => { void load(); }}>Повторить</button></div> : browse ? <>
        ...
        {tab === "selection" ? <>
          ...
          {candidateSelections.length > 0 && <div className={styles.shareComposer}>{readyShare ? <div className={styles.shareReady}><div><strong>Подборка создана</strong><small>{readyShare.items.length} объектов · ссылка действует 30 дней</small></div><div><button type="button" onClick={() => setBrowse(true)}>Редактировать</button><a href={readyShare.publicPath} target="_blank" rel="noopener noreferrer">Открыть</a><button className={styles.sharePrimary} type="button" onClick={() => { void copyShare(readyShare); }}>Копировать ссылку</button></div></div> : <><div><strong>Создать подборку</strong><span>Получите одну мобильную ссылку на выбранные объекты.</span></div><button className={styles.sharePrimary} type="button" disabled={busyKey === "create-share"} onClick={() => { void createShare(); }}>{busyKey === "create-share" ? "Создаём…" : "Создать ссылку"}</button></>}</div>}
        </> : <div className={styles.shareHistory}>{shares.map((share) => <article className={styles.shareHistoryItem} key={share.id}><div><span className={`${styles.shareStatus} ${styles[`shareStatus_${share.status}`]}`}>{shareStatusLabels[share.status]}</span><strong>{share.items.length} объектов</strong><small>{new Date(share.createdAt).toLocaleString("ru-RU")}{share.viewCount ? ` · открытий: ${share.viewCount}` : ""}</small><details><summary>Показать объекты</summary>{share.items.map((item) => <p key={item.id}>{item.title}{item.priceLabel ? ` · ${item.priceLabel}` : ""}</p>)}</details></div><div>{!(["REVOKED", "EXPIRED"] as string[]).includes(share.status) && <><a href={share.publicPath} target="_blank" rel="noopener noreferrer">Открыть</a><button type="button" onClick={() => { void copyShare(share); }}>Копировать</button><button type="button" disabled={busyKey === share.id} onClick={() => { void revokeShare(share); }}>Отозвать</button></>}</div></article>)}{!shares.length && <div className={styles.selectionState}>Созданных подборок пока нет.</div>}{legacyOffered.length > 0 && <p className={styles.legacySelectionNote}>Ранее вручную отмечено предложенными: {legacyOffered.length}.</p>}</div>}
      </>}
