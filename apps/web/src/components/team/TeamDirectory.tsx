...
type MemberTab = "overview" | "deals" | "properties" | "tasks" | "access";
...
type MemberProperty = { id: string; number: number; title: string; address: string | null; category: string; status: string };
type TeamRow = TeamMember & { pendingInvitationId: string | null; activeDeals: number; activeProperties: number; activeTasks: number; todayTasks: number; overdueTasks: number; deals: MemberDeal[]; properties: MemberProperty[]; tasks: MemberTask[] };

type TeamApiResponse = {
  members: Array<{
    id: string; name: string; email: string; phone: string | null; role: ApiRole; status: ApiStatus;
    joinedAt: string; updatedAt: string; pendingInvitationId: string | null; activeDeals: number; activeProperties: number; activeTasks: number; todayTasks: number; overdueTasks: number;
    deals: MemberDeal[]; properties: MemberProperty[]; tasks: MemberTask[];
  }>;
};

type InvitationLink = { invitationId: string; connectUrl: string; expiresAt: string; emailSent: boolean };
...
export function TeamDirectory() {
  ...
  return <section className={styles.page}>
    ...
    <main className={styles.content}>
      ...
      {loadState === "ready" && tab === "overview" && <Overview rows={rows} onOpen={openMember} onInvites={() => setTab("invites")} onDisabled={() => { setStatusFilter("Доступ отключён"); setTab("people"); }} onOverdue={() => router.push("/tasks?period=overdue")} />}
...
function Overview({ rows, onOpen, onInvites, onDisabled, onOverdue }: { rows: TeamRow[]; onOpen: (id: string) => void; onInvites: () => void; onDisabled: () => void; onOverdue: () => void }) {
  ...
  return <div className={styles.overview}>
    <section className={styles.composition}><header><div><h3>Состав команды</h3><p>Роли и состояние доступов</p></div><span>{active.length} участников</span></header><div className={styles.roleSummary}>{roles.map((role) => <div key={role}><strong>{active.filter((row) => row.role === role).length}</strong><span>{role === "Администратор" ? "Администратор" : role === "Руководитель" ? "Руководитель" : "Менеджеры"}</span></div>)}<div><strong>{rows.filter((row) => row.status === "Доступ отключён").length}</strong><span>Доступ отключён</span></div></div></section>
    <div className={styles.workspaceSurface}>
      ...
      <section className={styles.decisions}><header><div><h3>Требует решения</h3><p>Административные действия</p></div></header><div><button type="button" onClick={onOverdue}><strong>{overdueTasks}</strong><span><b>Просроченные задачи</b><small>{overdueMembers.length ? `У ${overdueMembers.length} сотрудников требуется внимание` : "У команды нет просрочек"}</small></span><i>›</i></button><button type="button" onClick={onInvites}><strong>{rows.filter((row) => row.status === "Приглашён").length}</strong><span><b>Ожидают приглашения</b><small>Показываются реальные членства со статусом ожидания</small></span><i>›</i></button><button type="button" onClick={onDisabled}><strong>{rows.filter((row) => row.status === "Доступ отключён").length}</strong><span><b>Доступ отключён</b><small>Проверить отключённых сотрудников и их задачи</small></span><i>›</i></button></div></section>
    </div>
  </div>;
}
...
function MemberPage({ member, initialTab, currentUserId, currentUserRole, onBack, onUpdated, onDeleted, assignees }: { member: TeamRow; initialTab: MemberTab; currentUserId: string; currentUserRole: ApiRole; onBack: () => void; onUpdated: () => Promise<void>; onDeleted: () => Promise<void>; assignees: TeamRow[] }) {
  ...
  const [offboardAction, setOffboardAction] = useState<"SUSPEND" | "DELETE" | null>(null);
  ...
  async function save() {
    ...
    if (status === "Доступ отключён" && member.status === "Активен") { setOffboardAction("SUSPEND"); return; }
    setSaving(true); setFeedback("");
    try {
      const response = await fetch(`/api/crm/team/${member.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: name.trim(), phone: phone.trim() || null, role: roleToApi[role], status: statusToApi[status] }) });
      const payload = await response.json() as { message?: string; error?: string };
      if (!response.ok) {
        if (payload.error === "offboarding_required") { setOffboardAction("SUSPEND"); return; }
        throw new Error(payload.message || "Не удалось сохранить сотрудника.");
      }
      await onUpdated(); setFeedback("Изменения сохранены.");
    } catch (caught) { setFeedback(caught instanceof Error ? caught.message : "Не удалось сохранить сотрудника."); }
    finally { setSaving(false); }
  }

  async function offboard(dealAssignments: Array<{ dealId: string; assigneeId: string }>, propertyAssignments: Array<{ propertyId: string; assigneeId: string }>) {
    if (!offboardAction) return;
    setSaving(true); setFeedback("");
    try {
      const response = await fetch(`/api/crm/team/${member.id}/offboard`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: offboardAction, dealAssignments, propertyAssignments }) });
      const payload = await response.json() as { message?: string; deferredDeals?: number; deferredProperties?: number };
      if (!response.ok) throw new Error(payload.message || "Не удалось изменить доступ сотрудника.");
      setOffboardAction(null);
      if (offboardAction === "DELETE") await onDeleted();
      else { await onUpdated(); setFeedback(payload.deferredDeals ? `Доступ отключён. Для ${payload.deferredDeals} сделок созданы задачи администратору.` : "Доступ отключён. Сделки распределены."); }
    } catch (caught) { setFeedback(caught instanceof Error ? caught.message : "Не удалось изменить доступ сотрудника."); }
    finally { setSaving(false); }
  }

  return <section className={styles.page}><header className={styles.topbar}><button className={styles.backLink} type="button" onClick={onBack}>← Команда</button><span className={styles.readOnlyNote}>{editable ? "Управление профилем и доступом" : "Просмотр профиля"}</span>{currentUserRole === "ADMIN" && member.id !== currentUserId && <button className={styles.deleteMember} type="button" disabled={saving} onClick={() => setOffboardAction("DELETE")}>Удалить сотрудника</button>}{editable && <button className={styles.saveMember} type="button" disabled={!changed || saving} onClick={() => { void save(); }}>{saving ? "Сохраняем…" : "Сохранить"}</button>}</header><main className={styles.memberPage}><div className={styles.memberHero}><span className={styles.heroAvatar}>{member.initials}</span><div><span>Участник команды</span><h2>{member.name}</h2><p>{member.email} · {member.role}</p></div><span className={`${styles.memberStatus} ${member.status === "Активен" ? styles.active : ""}`}>{member.status}</span></div>{feedback && <p className={styles.memberFeedback} role="status">{feedback}</p>}<nav className={styles.tabs}>{(["overview", "deals", "properties", "tasks", "access"] as MemberTab[]).map((item) => <button className={tab === item ? styles.tabActive : ""} type="button" onClick={() => setTab(item)} key={item}>{item === "overview" ? "Обзор" : item === "deals" ? `Сделки ${member.activeDeals}` : item === "properties" ? `Объекты ${member.activeProperties}` : item === "tasks" ? `Задачи ${member.activeTasks}` : "Доступ и уведомления"}</button>)}</nav>{tab === "overview" && <div className={styles.memberOverview}><section><h3>Рабочая нагрузка</h3><div className={styles.memberMetrics}><div><strong>{member.activeDeals}</strong><span>активных сделок</span></div><div><strong>{member.activeTasks}</strong><span>активных задач</span></div><div><strong>{member.overdueTasks}</strong><span>просрочено</span></div></div></section><section><h3>Профиль</h3><div className={styles.profileRows}><label><span>Имя</span><input readOnly={!editable} value={name} onChange={(event) => setName(event.target.value)} /></label><label><span>Email</span><input readOnly value={member.email} /></label><label><span>Телефон</span><input readOnly={!editable} value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Не указан" /></label><label><span>Роль</span><select disabled={!editable || member.id === currentUserId} value={role} onChange={(event) => setRole(event.target.value as TeamRole)}>{roles.map((item) => <option key={item}>{item}</option>)}</select></label><label><span>Статус</span><select disabled={!editable || member.id === currentUserId} value={status} onChange={(event) => setStatus(event.target.value as TeamStatus)}><option>Активен</option><option>Доступ отключён</option></select></label></div>{member.id === currentUserId && editable && <p className={styles.fieldNote}>Собственную роль и доступ должен изменять другой администратор. Это защищает рабочее пространство от случайной блокировки.</p>}</section></div>}{tab === "deals" && <EntityList items={member.deals.map((deal) => ({ title: `Сделка #${deal.number} · ${deal.title}`, subtitle: deal.request, meta: "Активна" }))} empty="Активных сделок нет" />}{tab === "properties" && <EntityList items={member.properties.map((property) => ({ title: `Объект ${property.number} · ${property.title}`, subtitle: property.address || "Адрес не указан", meta: property.category }))} empty="Назначенных объектов нет" />}{tab === "tasks" && <EntityList items={member.tasks.map((task) => ({ title: task.title, subtitle: task.contactName || task.dealTitle || "Без контакта", meta: task.dueDate ? `${task.dueDate}${task.dueTime ? `, ${task.dueTime}` : ""}` : "Без срока" }))} empty="Задач нет" />}{tab === "access" && <div className={styles.accessStack}><div className={styles.memberAccess}><header><h3>Права роли</h3><p>Базовый набор роли «{role}». Изменение роли применяется после сохранения профиля.</p></header>{(Object.keys(accessLabels) as AccessKey[]).map((key) => <label key={key}><span><strong>{accessLabels[key].title}</strong><small>{accessLabels[key].description}</small></span><input type="checkbox" checked={accessForRole(role)[key]} disabled readOnly /></label>)}</div><MemberNotificationSettings member={{ id: member.id, name: member.name, role: roleToApi[member.role] }} /></div>}</main>{offboardAction && <OffboardModal member={member} assignees={assignees} action={offboardAction} busy={saving} onCancel={() => setOffboardAction(null)} onConfirm={(assignments, propertyAssignments) => { void offboard(assignments, propertyAssignments); }} />}</section>;
}

function OffboardModal({ member, assignees, action, busy, onCancel, onConfirm }: { member: TeamRow; assignees: TeamRow[]; action: "SUSPEND" | "DELETE"; busy: boolean; onCancel: () => void; onConfirm: (assignments: Array<{ dealId: string; assigneeId: string }>, propertyAssignments: Array<{ propertyId: string; assigneeId: string }>) => void }) {
  const [assigned, setAssigned] = useState<Record<string, string>>({});
  const [bulkAssignee, setBulkAssignee] = useState("");
  const [propertyAssigned, setPropertyAssigned] = useState<Record<string, string>>({});
  const [bulkPropertyAssignee, setBulkPropertyAssignee] = useState("");
  const deferred = member.deals.filter((deal) => !assigned[deal.id]).length;
  return <div className={styles.modalLayer}><button className={styles.backdrop} type="button" aria-label="Закрыть" onClick={onCancel} /><section className={`${styles.modal} ${styles.offboardModal}`} role="dialog" aria-modal="true" aria-label={action === "DELETE" ? "Удаление сотрудника" : "Отключение доступа"}><header><div><span>{action === "DELETE" ? "Удаление сотрудника" : "Отключение доступа"}</span><h2>{action === "DELETE" ? `Удалить ${member.name}?` : `Приостановить доступ для ${member.name}?`}</h2></div><button type="button" onClick={onCancel} aria-label="Закрыть">×</button></header><div className={styles.modalBody}><p className={styles.modalHint}>{action === "DELETE" ? "Учётная запись и доступ будут удалены. Сделки, контакты и задачи сохранятся." : "Сотрудник выйдет из CRM и перестанет получать уведомления."}</p><div className={styles.suspensionFacts}><span><strong>{member.activeDeals}</strong> активных сделок</span><span><strong>{member.activeTasks}</strong> активных задач</span><span><strong>{member.activeProperties}</strong> объектов</span></div>{member.deals.length > 0 && <><label><span>Назначить все сделки одному сотруднику</span><select value={bulkAssignee} onChange={(event) => { const value = event.target.value; setBulkAssignee(value); setAssigned(Object.fromEntries(member.deals.map((deal) => [deal.id, value]))); }}><option value="">Выберите ответственного</option>{assignees.map((person) => <option value={person.id} key={person.id}>{person.name} · {person.role}</option>)}</select></label><div className={styles.offboardDeals}><strong>Сделки сотрудника</strong>{member.deals.map((deal) => <label key={deal.id}><span><b>#{deal.number} · {deal.title}</b><small>{deal.request}</small></span><select aria-label={`Ответственный за сделку #${deal.number}`} value={assigned[deal.id] || ""} onChange={(event) => setAssigned((previous) => ({ ...previous, [deal.id]: event.target.value }))}><option value="">Назначить позже</option>{assignees.map((person) => <option value={person.id} key={person.id}>{person.name}</option>)}</select></label>)}</div></>}{member.properties.length > 0 && <><label><span>Назначить все объекты одному сотруднику</span><select value={bulkPropertyAssignee} onChange={(event) => { const value = event.target.value; setBulkPropertyAssignee(value); setPropertyAssigned(Object.fromEntries(member.properties.map((property) => [property.id, value]))); }}><option value="">Назначить позже</option>{assignees.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label><div className={styles.offboardDeals}><strong>Объекты сотрудника</strong>{member.properties.map((property) => <label key={property.id}><span><b>OD-{property.number} · {property.title}</b><small>{property.address}</small></span><select value={propertyAssigned[property.id] || ""} onChange={(event) => setPropertyAssigned((previous) => ({ ...previous, [property.id]: event.target.value }))}><option value="">Назначить позже</option>{assignees.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label>)}</div></>}{deferred > 0 && <p className={styles.modalHint}>{deferred} {deferred === 1 ? "сделка останется" : "сделки останутся"} без ответственного. Администратору автоматически создастся {deferred === 1 ? "задача" : "по задаче на каждую сделку"}.</p>}{member.activeTasks > 0 && <p className={styles.modalHint}>Текущие задачи сотрудника останутся в CRM без ответственного.</p>}</div><footer><button type="button" onClick={onCancel}>Отмена</button><button className={styles.dangerButton} type="button" disabled={busy} onClick={() => onConfirm(member.deals.filter((deal) => assigned[deal.id]).map((deal) => ({ dealId: deal.id, assigneeId: assigned[deal.id] })), member.properties.filter((property) => propertyAssigned[property.id]).map((property) => ({ propertyId: property.id, assigneeId: propertyAssigned[property.id] }))) }>{busy ? "Сохраняем…" : action === "DELETE" ? "Удалить сотрудника" : "Отключить доступ"}</button></footer></section></div>;
}
...
function InvitationLinkModal({ invitation, onClose }: { invitation: InvitationLink; onClose: () => void }) {
  ...
  return <div className={styles.modalLayer}><button className={styles.backdrop} type="button" aria-label="Закрыть" onClick={onClose} /><section className={styles.modal}><header><div><span>Приглашение создано</span><h2>{invitation.emailSent ? "Письмо передано на отправку" : "Передайте ссылку сотруднику"}</h2></div><button type="button" onClick={onClose}>×</button></header><div className={styles.modalBody}><p className={styles.modalHint}>{invitation.emailSent ? "Ссылка отправлена через почтовый сервис. На случай задержки письма её можно скопировать здесь." : "Почтовая отправка пока не настроена или не удалась. Скопируйте ссылку и отправьте её сотруднику вручную."} Ссылка одноразовая и действует 7 дней.</p><label><span>Ссылка приглашения</span><input readOnly value={invitation.connectUrl} onFocus={(event) => event.currentTarget.select()} /></label></div><footer><button type="button" onClick={onClose}>Готово</button><button className={styles.modalPrimary} type="button" onClick={() => void copy()}>{copied ? "Скопировано" : "Скопировать ссылку"}</button></footer></section></div>;
}
