...
function MemberPage({ member, initialTab, currentUserId, currentUserRole, onBack, onUpdated, onDeleted, assignees }: { member: TeamRow; initialTab: MemberTab; currentUserId: string; currentUserRole: ApiRole; onBack: () => void; onUpdated: () => Promise<void>; onDeleted: () => Promise<void>; assignees: TeamRow[] }) {
  ...
  async function offboard(dealAssignments: Array<{ dealId: string; assigneeId: string }>) {
    if (!offboardAction) return;
    setSaving(true); setFeedback("");
    try {
      const response = await fetch(`/api/crm/team/${member.id}/offboard`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: offboardAction, dealAssignments }) });
      const payload = await response.json() as { message?: string; deferredDeals?: number };
