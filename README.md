...
Следующий этап проекта — подключение фотографий через Cloudflare R2. Для него настройте на API-сервисе `R2_ACCOUNT_ID`, `R2_BUCKET`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` и CORS бакета для `PUT` с домена CRM. Пока R2 не подключён, карточки и Excel работают без фотографий. Правила формата, сохранения исходных колонок и настройки бакета описаны в [документации вторичных объектов](docs/SECONDARY_PROPERTY_CATALOG_DESIGN.md).
