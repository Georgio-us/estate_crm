...
CRM для агентств недвижимости. Репозиторий организован как monorepo: рабочий frontend находится в `apps/web`, Fastify API — в `apps/api`, а схема PostgreSQL и миграции Prisma — в `packages/database`. Авторизация, контакты, воронка, задачи, календарь, настройки, подборки объектов в сделках и первый реальный поток Meta Lead Ads для Delmar подключены к постоянному хранению. Каталог содержит рабочие объекты организации; демо-каталог удалён. Подписка остаётся интерфейсной моделью.

Актуальная граница готовности — **рабочий пилот**: системой уже пользуется команда примерно из восьми сотрудников. Основной разработческий контур готов: лиды, роли, контакты, сделки, задачи, рабочий каталог, архив объектов, публичные подборки и первый уровень каталога новостроек. Новостройки организованы как отдельные сущности «застройщик → проект» с независимыми статусами строительства и продаж; корпуса, шахматка, юниты и документы проекта относятся к следующему согласованному слою. Проверка командой, безопасность развёртывания и восстановление из резервной копии относятся к приёмке и эксплуатации. Instagram Direct, телефония и платежи не входят в текущий обязательный объём.
...
- В неразобранной карточке телефон не отдаётся ни одной роли. Кнопка «Взять в работу» одинаково доступна администратору, руководителю и менеджеру: можно назначить себя или активного коллегу либо отменить действие.
...
Публичная ссылка на подборку показывает клиенту сохранённые в базе цену и валюту без автоматической конвертации, площадь, комнаты и дополнительные параметры по типу объекта. По нажатию открываются описание и галерея доступных фотографий; внизу страницы выводятся имя, телефон и email ответственного за сделку менеджера. Сами фотографии остаются закрытыми в R2 и выдаются только через действующую ссылку подборки.

Интеграция Via сохранена как отключаемый модуль, но для Delmar выключена 22 сентября 2026 года. Внешний каталог Via не загружается, новые импорты и входящие события не принимаются. Ранее импортированные объекты остаются обычными объектами CRM и не удаляются при выключении интеграции.
...
- сохранённый, но выключенный контракт Via: [`docs/VIA_INTEGRATION_CURRENT_EXECUTION.md`](docs/VIA_INTEGRATION_CURRENT_EXECUTION.md).
